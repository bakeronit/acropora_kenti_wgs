---
title: "Detecting selection based on PCA (PCAngsd)"
...
p1<-
  ...
  geom_text(data=pcangsd_df %>% left_join(offsets) %>% mutate(abs_pos=pos+offset) %>% filter(pval<=1e-10) %>% group_by(chr) %>% filter(pval==min(pval)),
             aes(x=abs_pos/1e+6,y=-log10(pval)),label=c("L2","L1","L3"),color="black",hjust=-0.3) + 
...
p3<-pc_plots(l2_covmat)[3][[1]] + xlab("L2")
p4<-pc_plots(l3_covmat)[3][[1]] + xlab("L3")

p1/(p2|p3|p4)

#ggsave("fig-2.3.png",height = 3.7,width = 6.75)

```

### Attempt to do GO enrichment

I don't feel GO enrichment of these genes is a useful idea since the decision of including which genes is a bit arbitrary. A simple table of genes involved should be good enough.
