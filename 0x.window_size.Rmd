...
Since some of those extreme Fst values are from windows with a lot of missing data, we plotted the window-based Fst values along the chromosomes with three filtering thresholds for missing data: 10%, 50%, 80%.
...
```{r}
theta_df %>% left_join(offsets) %>% mutate(abs_pos=pos+offset) %>% filter(covered>0.05) %>% 
  mutate(chr_type=ifelse(scaffold_num %% 2 == 0,"dark", "light")) %>% mutate(chr_type=ifelse(grepl(chr,pattern="chr"),chr_type,"unplanced")) %>% 
...
**Figure 7** Manhattan plot of nucleotide diversity(pi) across the genome excluding windows with more than 95% of missing data